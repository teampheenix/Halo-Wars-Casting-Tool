...
# create logger
module_logger = logging.getLogger('hwctool.view.widgets')
...
class MapLineEdit(QLineEdit):
    ...
    def __handleEditingFinished(self):
        ...
        if before != after:
            after, known = hwctool.matchdata.autoCorrectMap(after)
            self.setText(after)
            self._before = after
...
class StyleComboBox(QComboBox):
    ...
    def __init__(self, style_dir, scope):
        ...
        self.__scope = scope
        style_dir = hwctool.settings.getAbsPath(style_dir)
        default = hwctool.settings.config.parser.get("Style", self.__scope)
    ...
    def save(self):
        ...
        #     new_file = hwctool.settings.getAbsPath(new_file)
        #     shutil.copy(new_file, hwctool.settings.getAbsPath(file))
...
class MapDownloader(QProgressDialog):
    ...
    def __init__(self, mainWindow, map_name, url):
        ...
        mapdir = hwctool.settings.getAbsPath(hwctool.settings.casting_html_dir)
...
class HotkeyLayout(QHBoxLayout):
    ...
    def __init__(self, parent, ident, label="Hotkey", hotkey=""):
        """Init box."""
        super().__init__()
        self.data = hwctool.settings.config.loadHotkey(hotkey)
...
class InitialUpdater(QProgressDialog):
    ...
    def run(self):
        ...
        try:
            from hwctool.settings.client_config import ClientConfig
            from hwctool.tasks.updater import extractData
            from pyupdater.client import Client
            client = Client(ClientConfig())
            client.refresh()
            client.add_progress_hook(self.setProgress)

            channel = hwctool.tasks.updater.getChannel()
            lib_update = client.update_check(
                hwctool.tasks.updater.VersionHandler.ASSET_NAME,
                self.version,
                channel=channel)
...
class DragDropLogoList(QListWidget):
    def __init__(self, logoManager, addIdent=lambda: None):
        ...
        self._iconsize = hwctool.settings.logoManager.Logo._iconsize
...
class ProfileMenu(QMenu):
    ...
    def __init__(self, parrent_widget, controller):
        ...
        action = self._menu.addAction(QIcon(hwctool.settings.getResFile(
        ...
        for profile in hwctool.settings.profileManager.getProfiles():
    ...
    def removeProfile(self):
        ...
        try:
            hwctool.settings.profileManager.deleteProfile(profile['id'])
    ...
    def openFolder(self):
        os.startfile(hwctool.settings.profileManager.profiledir())
    ...
    def duplicateProfile(self):
        ...
        while True:
            ...
            try:
                ...
                id = hwctool.settings.profileManager.addProfile(
    ...
    def importProfile(self):
        ...
        while True:
            ...
            QApplication.setOverrideCursor(Qt.WaitCursor)
            try:
                id = hwctool.settings.profileManager.importProfile(
                    filename, name)
                self.addProfile(id, name, False)
                self.selectProfile(id)
                pass
    ...
    def importProfileOverwrite(self):
        ...
        QApplication.setOverrideCursor(Qt.WaitCursor)
        try:
            hwctool.settings.profileManager.deleteProfile(id, True)
            id = hwctool.settings.profileManager.importProfile(
                filename, name, id)
            self.addProfile(id, name, False)
            self.selectProfile(id)
    ...
    def selectProfile(self, myid):
        ...
        hwctool.settings.profileManager.setDefault(myid)
        # hwctool.settings.profileManager.setCurrent(myid)
...
class ScopeGroupBox(QGroupBox):
    ...
    def __init__(self, name='', options=list(), scope='all', parent=None):
        ...
        for set_idx in range(0, hwctool.settings.max_no_sets):
    ...
    def adjustRangeUpper(self, lower):
        ...
        self.cb_upper.clear()
        rg = range(lower, hwctool.settings.max_no_sets)
